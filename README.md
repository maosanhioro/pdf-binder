...
## 概要
PdfBinder は、複数PDFの結合／ページ抽出を、迷わず操作できるGUIで提供するツールです。
配布先では Python の導入が不要なため、非技術者の利用にも適しています。

## できること
- PDF結合: 複数PDFファイルを1つにまとめる
- ページ抽出: 1つのPDFから指定ページだけを新しいPDFとして保存する
- 出力先・出力名の指定
- 既存ファイルへの上書き確認

## 使い方（配布版）
1. `dist/PdfBinder.exe` を起動します。
2. 左側のタブで「まとめる」「抜き出し」を選びます（抜き出し＝ページ抽出）。
